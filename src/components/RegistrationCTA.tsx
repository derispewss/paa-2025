...
const RegistrationCTA = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (now < START_DATE) {
        ...
        setTimeLeft(calcTime(START_DATE - now));
      } else if (now >= START_DATE && now <= END_DATE) {
        setRegistrationStatus("open");
        setTimeLeft(calcTime(END_DATE - now));
      } else {
  ...
  const handleRegistration = () => {
    if (registrationStatus === "open") {
      window.open(FORM_REGIST, "_blank");
    ...
    switch (registrationStatus) {
      ...
      case "closed":
        return {
          ...
          subtitle:
            "Pendaftaran telah berakhir. Nantikan informasi selanjutnya!",
  ...
  return (
    <section
      id="registration"
      className="w-full relative py-8 sm:py-12 lg:py-16 mb-8 sm:mb-12 lg:mb-16"
      ref={sectionRef}
    >
          ...
          >
            <p className="text-foreground text-center px-2 sm:px-4 text-sm sm:text-base lg:text-lg leading-relaxed">
              {registrationStatus === "closed" ? (
                <>
                  Pendaftaran PAA 2025 telah berakhir. Terima kasih kepada semua
                  yang telah mendaftar!
                  <span className="text-accent font-bold">
                    {" "}
                    Nantikan pengumuman hasil seleksi{" "}
                  </span>
                ...
                <>
                  ...
                  <span className="text-accent font-bold">
                    {" "}
                    Pembekalan Anggota Aktif 2025
                  </span>{" "}
                  dan mulai perjalanan menuju masa depan yang lebih cerah bersama
                  HMTI.
                </>
              )}
            </p>
          ...
          {registrationStatus !== "closed" && (
            ...
            >
              {[
                ...
                <div
                  key={index}
                  className="bg-primary/30 backdrop-blur-sm border border-accent/20 rounded-lg sm:rounded-xl p-2 sm:p-4 text-center hover:bg-primary/50 transition-all duration-300 hover:scale-105"
                >
                  <div className="text-lg sm:text-2xl lg:text-3xl font-bold text-accent">
